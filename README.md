# Docker image for single-cell RNAseq analysis using Seurat | SingleR | Celldex
Tha package is available at docker hub. You can pull it as:
```
docker pull pritampkp15/rpackages-scrna
```
...
```
## Commands Used to build the container for macos:
```
docker build -t rpackages-scrna:latest .
docker tag rpackages-scrna:latest pritampkp15/rpackages-scrna:latest
docker push pritampkp15/rpackages-scrna:latest
```
## Commands Used to build the container for linux/amd
```
docker build --platform linux/amd64 -t pritampkp15/rpackages-scrna:latest --push .
```
